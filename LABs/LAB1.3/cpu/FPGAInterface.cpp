...
void FPGAInterface::gamma(int* v, double gamma)
{
    cl_int ret;
    
    ret = clSetKernelArg(m_gammaKernel, 0, sizeof(cl_int), (void *)v);
    SAMPLE_CHECK_ERRORS(ret);
    
    ret = clSetKernelArg(m_gammaKernel, 1, sizeof(cl_double), (void *)&gamma);
    SAMPLE_CHECK_ERRORS(ret);
    
    ret = clSetKernelArg(m_gammaKernel, 2, sizeof(cl_mem), (void *)&m_memOutV);
    SAMPLE_CHECK_ERRORS(ret);
